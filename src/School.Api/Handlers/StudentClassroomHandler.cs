...
{
    public async Task<Response<Dictionary<string, string>>> AssociateAsync(CreateStudentClassroomAssociationRequest request)
    {
        var association = await AssociationExistsAsync(request.StudentUser, request.Classroom);
        if (association is true)
            return new Response<Dictionary<string, string>>(null, 400, "Aluno já está cadastrado na turma");

        var result = await repository.AssociateAsync(request.StudentUser, request.Classroom);
        
        return new Response<Dictionary<string, string>>(result, 201, "Associação feita com sucesso!");
    }
